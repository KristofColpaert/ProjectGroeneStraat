...
function on_plugin_activated_readygraph_rpr_redirect(){
	global $rpr_menu_slug;
	$setting_url="admin.php?page=$rpr_menu_slug";    
...
function add_rpr_readygraph_plugin_warning() {
  if (get_option('readygraph_access_token', '') != '') return;
  global $hook_suffix, $current_user, $rpr_menu_slug;
  ...
  if ( $hook_suffix == 'plugins.php' && get_option('readygraph_connect_notice') == 'true' ) {              
    echo '<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">  
      ...
      <form name="readygraph_activate" action="'.admin_url( 'admin.php?page=' . $rpr_menu_slug).'" method="POST"> 
	...
	add_action('admin_notices', 'add_rpr_readygraph_plugin_warning');
	...
	add_action('wp_footer', 'rpr_readygraph_client_script_head', 9);
	}
	}
...
function rpr_post_updated_send_email( $post_id ) {
	...
	if(get_option('rpr_wordpress_sync_users')) return;
	else {
		add_action('plugins_loaded', 'rpr_wordpress_sync_users');
	}
function rpr_wordpress_sync_users() {
	if(get_option('rpr_wordpress_sync_users') && get_option('rpr_wordpress_sync_users') == "true") return;
	else {
		if(get_option('readygraph_application_id') && strlen(get_option('readygraph_application_id')) > 0){
			global $wpdb;
			$app_id = get_option('readygraph_application_id');
			...
			remove_action('plugins_loaded', 'rpr_wordpress_sync_users');
		}
    }
}
function rpr_rg_connect(){
	if(get_option('readygraph_connect_anonymous') != "true"){
	$url = 'https://readygraph.com/api/v1/wordpress-rg-connect-anonymous/';
	$randon_string = rpr_get_random_string();
...
function rpr_get_random_string()
{
