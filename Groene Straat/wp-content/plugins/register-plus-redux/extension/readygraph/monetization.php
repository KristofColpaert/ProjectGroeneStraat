...
include("header.php"); 
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		rpr_readygraph_monetize_update();
	}
?>	
...
<input type="hidden" name="readygraph_popup_template" value="<?php echo get_option('readygraph_popup_template', 'default-template') ?>">
	...
	<div style="margin: 3% 5%">
		<?php if(get_option('readygraph_enable_monetize') && get_option('readygraph_enable_monetize') == "true") { ?><h3 style="font-weight: normal; text-align: center;"><img src="<?php echo plugin_dir_url( __FILE__ );?>assets/check.png"/>Congratulations! <?php echo $rpr_main_plugin_title; ?>'s ReadyGraph monetization engine is now active.</h3><?php } ?>
